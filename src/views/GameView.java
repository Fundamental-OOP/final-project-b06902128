...
public class GameView extends JFrame {
    ...
    public GameView(Game game) throws HeadlessException {
        this.game = game;
        game.setView(canvas);
        this.holynova = new Holynova(game.getPlayer());
        this.block = new Block(game.getPlayer());
        this.boost = new Boost(game.getPlayer());
        this.power = new Power(game.getPlayer());
        this.heal = new Heal(game.getPlayer());
    }
            ...
            public void keyPressed(KeyEvent keyEvent) {
                switch (keyEvent.getKeyCode()) {
                    ...
                    case KeyEvent.VK_K:
                        ...
                        break;
                    case KeyEvent.VK_L:
                        skill = game.useAbility("holynova");
                        break;
                    case KeyEvent.VK_U:
                        skill = game.useAbility("block");
                        break;
                    case KeyEvent.VK_I:
                        skill = game.useAbility("boost");
                        break;
                    case KeyEvent.VK_O:
                        skill = game.useAbility("power");
                        break;
                    case KeyEvent.VK_P:
                        skill = game.useAbility("heal");
                        break;
    ...
    public class Canvas extends JPanel implements GameLoop.View {
        ...
        protected void paintComponent(Graphics g /*paintbrush*/) {
            ...
            if (skill != null) {
                if (skill.stop()) {
                    skill.render(g, skillNum);
                }
                else if (skillNum % 30 < 15){
                    skill.render(g);
                }
                if (++skillNum == 75) {
                    if (skill.stop()) {
